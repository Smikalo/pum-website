...
router.post("/contact", async (req, res) => {
    ...
    if (!email) return sendBadRequest(res, "Invalid email address.");
...
router.post("/newsletter/subscribe", async (req, res) => {
    ...
    try {
        ...
        sendServerError(res, "Failed to subscribe. Please try again later.");
    }
});
...
router.post("/newsletter/verify", async (req, res) => {
    ...
    if (!parsed.success) return sendBadRequest(res, "Invalid verification token.", { code: "INVALID_INPUT" });
    ...
    try {
        ...
        return sendBadRequest(res, isExpired ? "This verification link has expired." : "This verification link is invalid.", { code: isExpired ? "TOKEN_EXPIRED" : "TOKEN_INVALID" });
    }

    if (!decoded || decoded.scope !== "newsletter-verify") return sendBadRequest(res, "This link is not valid for newsletter verification.", { code: "BAD_SCOPE" });
    ...
    try {
        ...
        if (!subscriber) return sendBadRequest(res, "We couldn’t find a matching subscription for this link.", { code: "NOT_FOUND" });

        const subEmailLower = (subscriber.email || "").toLowerCase();
        if (tokenEmail && tokenEmail !== subEmailLower) return sendBadRequest(res, "This verification link does not match this subscription.", { code: "EMAIL_MISMATCH" });

        if (subscriber.verifiedAt) return res.json({ ok: true, status: "already-verified", email: subscriber.email });
        if (subscriber.unsubscribedAt) return sendBadRequest(res, "This subscription was cancelled and cannot be verified.", { code: "UNSUBSCRIBED" });
        ...
        sendServerError(res, "Failed to verify subscription. Please try again later.", { code: "SERVER_ERROR" });
    }
});
...
router.post("/newsletter/unsubscribe", async (req, res) => {
    ...
    if (!parsed.success) return sendBadRequest(res, "Invalid unsubscribe token.", { code: "INVALID_INPUT" });
    ...
    try {
        ...
        const isExpired = err && err.name === "TokenExpiredError";
        return sendBadRequest(res, isExpired ? "This unsubscribe link has expired." : "This unsubscribe link is invalid.", { code: isExpired ? "TOKEN_EXPIRED" : "TOKEN_INVALID" });
    }

    if (!decoded || decoded.scope !== "newsletter-unsub") return sendBadRequest(res, "This unsubscribe link is not valid for newsletter settings.", { code: "BAD_SCOPE" });
    ...
    try {
        ...
        const subEmailLower = (subscriber.email || "").toLowerCase();
        if (tokenEmail && tokenEmail !== subEmailLower) return sendBadRequest(res, "This unsubscribe link does not match this subscription.", { code: "EMAIL_MISMATCH" });
        ...
        sendServerError(res, "Failed to update subscription. Please try again later.", { code: "SERVER_ERROR" });
    }
});
